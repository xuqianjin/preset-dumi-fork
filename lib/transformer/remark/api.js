...
function guessComponentScope(fileAbsPath) {
  const parsed = _path().default.parse(fileAbsPath);

  return _path().default.basename(parsed.dir.replace(/(\/|\\)(src|lib)\S*/, ''));
  ...
  return (ast, vFile) => {
    (0, _unistUtilVisit().default)(ast, 'element', (node, i, parent) => {
      if ((0, _hastUtilIsElement().default)(node, 'API') && !node._dumi_parsed) {
        let scope = guessComponentScope(this.data('fileAbsPath'));
        let identifier;
        let definitions;
        ...
        if ((0, _hastUtilHasProperty().default)(node, 'src')) {
          ...
          definitions = (0, _apiParser.default)(absPath, parseOpts);
          identifier = scope + (componentName || src); // trigger listener to update previewer props after this file changed
          ...
          try {
            ...
            identifier = scope + vFile.data.componentName; // trigger listener to update previewer props after this file changed
